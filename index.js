...
function getResult(moves) {
    if (moves.userMove == moves.cpuMove) {
        ...
        (moves.userMove == "r" && moves.cpuMove == "s") ||
        (moves.userMove == "p" && moves.cpuMove == "r") ||
        (moves.userMove == "s" && moves.cpuMove == "p")
    ) {
        return 1;
    } else {
        return -1;
...
function main() {
    ...
    const stageHand1 = document.querySelector(
        ".middle__result-container__stage__hand1"
    );
    const stageHand2 = document.querySelector(
        ".middle__result-container__stage__hand2"
    );
    const resultUpdateText = document.querySelector(
        ".middle__result-container__updation-text"
    );
    const resultDetailsText = document.querySelector(
        ".middle__result-container__details-text"
    );
    ...
    middleTitle.addEventListener("click", function () {
        ...
        showDiv(userScoreDisplay, "block");
        showDiv(cpuScoreDisplay, "block");
        showDiv(header, "flex");
    ...
    function updateResultDetails() {
        if (moves.userMove == moves.cpuMove) {
            ...
            (moves.userMove == "r" && moves.cpuMove == "p") ||
            (moves.userMove == "p" && moves.cpuMove == "r")
        ) {
            addTypeWriterEffect(
                resultDetailsText,
            ...
            (moves.userMove == "r" && moves.cpuMove == "s") ||
            (moves.userMove == "s" && moves.cpuMove == "r")
        ) {
            addTypeWriterEffect(
                resultDetailsText,
                "rock beats scissors!!",
            ...
            (moves.userMove == "s" && moves.cpuMove == "p") ||
            (moves.userMove == "p" && moves.cpuMove == "s")
        ) {
            addTypeWriterEffect(
                resultDetailsText,
    ...
    function updateScore(res) {
        if (res == 1) {
            ...
            score.userScore += 1;
            userScoreDisplay.innerHTML = `user_score: ${score.userScore}`;
        } else if (res == -1) {
            let scoreDigits = findDigits(score.cpuScore);
            score.cpuScore += 1;
            cpuScoreDisplay.innerHTML = `cpu_score: ${score.cpuScore}`;
        }
    }
